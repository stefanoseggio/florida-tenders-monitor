...
**Preferred: GitHub Private Vulnerability Reporting.** This repository has private vulnerability reporting enabled — go to the **Security** tab → **Report a vulnerability** to open a private advisory visible only to the maintainer until a fix is ready. This is the correct channel for anything that shouldn't be disclosed in a public issue (dependency CVEs affecting this Actor's real usage, request-handling issues, etc.).
...
- **No credential handling of any kind.** This Actor requires no third-party API key or BYOK secret (see the README's Cost & BYOK Disclosure section) — it needs nothing beyond your Apify account. There is no separate MyFloridaMarketPlace credential and no pooled or resold third-party license. There is no customer secret this Actor could leak.
- **No user-supplied code execution.** Input is a fixed JSON schema (`statuses`, `types`, `agencyIds`, `commodityCodes`, `dateFrom`/`dateTo`, `closesAfter`, `onlyNew`, `maxItems`, `fetchDetail`, etc.) validated against `.actor/input_schema.json` and the runtime checks in `src/input.ts` — there is no arbitrary-code or arbitrary-URL input surface.
...
- **Source-portal integrity** (a compromised or spoofed MyFloridaMarketPlace endpoint) is outside this Actor's control — it fetches from the portal's own official `vendor.myfloridamarketplace.com` domain (operated by the Florida Department of Management Services) over HTTPS and does not implement independent content-signing verification beyond standard TLS.
