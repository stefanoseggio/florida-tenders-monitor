...
describe.skipIf(process.env.CI)('live fetchTenders against the real MFMP portal', () => {
    it('fetches real OPEN solicitations with full detail', async () => {
        const { results: tenders } = await fetchTenders(['OPEN'], true, 5, new Set(), false, undefined, new Date());
        ...
        for (const t of tenders) {
            ...
            expect(t.is_new).toBe(true); // cold call, empty seen-set
        }
    ...
    it('paginates across multiple pages when maxItems exceeds one page', async () => {
        const { results: tenders } = await fetchTenders(
            ['CLOSED'],
            false,
            150,
            new Set(),
    ...
    it('respects maxItems as a hard cap', async () => {
        const { results: tenders } = await fetchTenders(['CLOSED'], false, 5, new Set(), false, undefined, new Date());
        expect(tenders.length).toBeLessThanOrEqual(5);
    }, 30_000);
});
